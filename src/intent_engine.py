...
def classify(phrase: str, known_keys: list[str]) -> dict:
    ...
    prompt = f"""Eres un clasificador de intenciones para automatización del hogar.
Dado el texto del usuario, responde SOLO con un JSON válido.

Intenciones posibles: {intents}

Extrae también los parámetros mencionados explícitamente:
- nombres, números, estados (encender/apagar, activar/desactivar, abrir/cerrar)
- temperatura en grados, porcentajes, zonas, comandos de sistema

Formato: {{"intent": "nombre", "params": {{...}}}}
Solo incluir parámetros mencionados. Si no encaja en ninguna intención: "desconocido".
